...
sub connect {
   ...
   my $sockpath = shift;
   my $count;
   until($self->{sock} = IO::Socket::UNIX->new(Type => SOCK_STREAM, Peer => $sockpath)) {
...
sub lock {
   ...
   my $resp = 0;
   $resp = $self->send("LOCK:" . (join ":", $self->get_id, $tid)), $/ while not $resp and usleep 5;
}
...
sub value_or_id {
   ...
   my $obj;
   my $type;
   if(my $ref = ref $value or $value =~ /^(?:fer(SCALAR|ARRAY|HASH)\(\d+\))$/) {
      $type = $1;
