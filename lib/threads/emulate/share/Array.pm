...
sub STORE {
   ...
   my $resp = $self->send("STORE:" . ($self->get_id) . ":$index:" . $self->value_or_id($value));
...
sub PUSH {
   ...
   my @value = @_;
   my @value_;
   push @value_, ref $_ ? $self->value_or_id(threads::emulate::share::easyshare_attr($_)) : $_ for @value;
   $self->lock(&main::get_tid);
   my $resp = $self->send("PUSH:" . ($self->get_id) . ":" . (join ":", @value_));
   $self->unlock(&main::get_tid);
   $resp
}
...
sub UNSHIFT {
   ...
   my @value = @_;
   push @value_, ref $_ ? $self->value_or_id(threads::emulate::share::easyshare_attr($_)) : $_ for @value;
   my $resp = $self->send("UNSHIFT:" . ($self->get_id) . ":" . (join ":", @value_));
   $self->unlock(&main::get_tid);
   $resp
}
...
sub SPLICE {
   ...
   my $length = shift;
   my @value  = @_;
   my @value_ = ref $_ ? threads::emulate::share::easyshare_attr($_) : $_ for @value;
   $self->lock(&main::get_tid);
   my $resp = $self->send("SPLICE:" . ($self->get_id) . ":" . (join ":", $offset, $length, @value_));
   $self->unlock(&main::get_tid);
   $resp
}
